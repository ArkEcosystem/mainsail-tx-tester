...
export class TransferBuilder implements ITransferBuilder {
    ...
    public async makeTransfer(
        ...
        const signed = await this.signTransaction(builder, cli);

        return signed.build();
    }

    protected async getAddress(cli: any): Promise<string> {
    ...
    protected async getWalletNonce(config: Config): Promise<number> {
        ...
        const walletAddress = await this.getAddress(config.cli);
